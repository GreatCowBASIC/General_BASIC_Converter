...
The converter performs a best-effort, source-to-source translation from General BASIC style syntax into GCBASIC syntax. It is designed to automate the repetitive mechanical changes that come up during a port, while leaving ambiguous or unsafe constructs as review markers instead of silently guessing.

## Usage

Source files must have the extension `.prt`. Rename source files from `.bas` / `.txt` etc. to `.prt` before conversion.
...
Usage from within GCBASIC
    ...
    #include "source.prt"
    #option Explicit
    // This will create an output file that is then included as a source.
...
1. **Pre-passes over the whole file**, before any line-by-line rewriting:
   ...
   - Collect GOSUB targets so only those labels become `Sub`/`End Sub` (plain mid-sub labels are left alone).
...
3. **Translate literals, statements, and control-flow forms.**
   - `DelayMS n` → `Wait n ms`.
   - `%binary` literals → `0b...`.
   - `$hex` literals (any length: `$FF`, `$1F80`, `$FFFFFFFF`, ...) → `0x...`. Applied once, early, right after the code/comment split, so every other rewrite path automatically sees already-converted `0x` literals.
   ...
   - `Select Case` comparisons like `Case > n` / `Case < n` → `Case n To <upper-bound>` / `Case <lower-bound> To n - 1`, with the bound inferred from the variable's declared type where possible.
   - `GOSUB name` / `GoSub name` → bare `name` (Sub call), including mid-line forms.
   - `label:` … `Return` for GOSUB targets only → `Sub name` … `End Sub`. Intermediate `Return` → `Exit Sub`; the final `Return` is absorbed into `End Sub`. Plain mid-sub labels (e.g. `CFG:`) stay as labels and do not close the Sub.
...
7. **Serial commands.** GCBASIC has no General BASIC-style bracketed item-list serial statements, so:
   - `HSerOut [a, b, ...]` (whole line) → one `HSerSend` / `HSerPrint` call per item as appropriate.
   - Residual mid-line `HSerOut[...]` (e.g. after `Then`) → neutralized with `NOP // HSerOut[...]` and a `>>> REVIEW` marker so the line still compiles pending manual rewrite.
   - `HSerIn timeout,label,[a, b]` → flagged for review (no safe automatic timeout/STR equivalent).
...
- the transformation is only an approximation (e.g. residual mid-line `HSerOut`),
- the original construct has no direct GCBASIC equivalent (e.g. `SerIn`/`HSerIn` with a timeout/label),
- the converter cannot safely infer the target type or range (e.g. an unbounded `Select Case >` comparison),
- a `Dim` has a trailing modifier that isn't a recognized GCBASIC option (e.g. General BASIC `Heap`) and its intended purpose isn't guessed at,
...
The following helpers may be appended automatically to the output file when needed:

- `EE_ReadLong` / `EE_WriteLong` — if the source uses `ERead`/`EWrite` on a Long/Dword-typed variable (GCBASIC's `EPRead`/`EPWrite` only handle one EEPROM byte at a time).
- `ArrayToString` — if the source uses General BASIC's bareword `Str arrayvar` form to copy a byte array into a string.
...
4. Fix any remaining syntax or semantic issues manually — in particular residual `HSerOut`/`HSerIn`/`SerIn` timeout forms, and any Dim modifier flagged as unrecognized.
