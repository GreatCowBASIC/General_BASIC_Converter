#!/usr/bin/env python3
"""
picbasic_to_gcbasic.py
=======================

Heuristic source-to-source converter for porting PicBasic-Pro style code to
Great Cow BASIC (GCBASIC).
...
- Common serial and EEPROM helpers for PicBasic idioms
...
Usage
-----
    python picbasic_to_gcbasic.py source.bas
    python picbasic_to_gcbasic.py source.bas -o my_output.gcb
    python picbasic_to_gcbasic.py source.bas --xtal 20
...
----------------------
1. Normalize source lines and strip or rewrite obvious PicBasic syntax.
...
# is optional since PicBasic accepts either. Doesn't match "Declare
...
# PicBasic-ism with no GCBASIC meaning, so it's dropped entirely
# ("Config5 CP_OFF" -> "#Config CP_OFF").
RE_CONFIG_LINE = re.compile(r'^(\s*)Config[1-9]\d*\b(.*)$', re.IGNORECASE)

# --- Serial commands -------------------------------------------------
# GCBASIC has no PicBasic-style bracketed item-list serial statements.
...
# available approximation of PicBasic's per-byte timeout, flagged below
# since it isn't exact and 255 can also be a genuine received byte).
# --- Command lookup table ---------------------------------------------
# A single, easily-extended table of "old PicBasic construct -> new
...
# PicBasic hex literals ($FF, $FFFF, $1F80, ...) -> GCBASIC's 0x prefix.
...
# PicBasic allows a space between an array name and its size, e.g.
...
# modifier (Alias / At / = initialvalue) - e.g. the PicBasic-Photon
...
# PicBasic's "For var = start UpTo end" is GCBASIC's "For var = start To
...
# Not X, A Or B, A Xor B). A PicBasic Symbol constant that happens to be
...
# PicBasic Pro Pascal-style block comments: (* ... *). GCBASIC has no
...
' EEPROM byte at a time, unlike PicBasic-style ERead/EWrite which
...
def convert_line(line, var_types, needs_helpers, lookup_usage, select_case_var=None):
    ...
    # PicBasic hex literals ($FF, $1F80, ...) -> GCBASIC's 0x prefix.
    ...
    # PicBasic-Photon "Heap" qualifier, which has no known GCBASIC
    ...
    # (255) is the closest available stand-in for PicBasic's per-byte
...
def convert_block_comment_line(line, in_block_comment):
    """Handle one line that opens, continues, or closes a PicBasic-Pro
    ...
    def review(reason_line):
        return (f"' >>> REVIEW: line commented out - PicBasic (* *) block "
...
def find_array_declarations(lines):
    """Scan for PicBasic-style array declarations: 'Dim NAME [SIZE] As
...
def parse_picbasic_int(token):
    """Parse a PicBasic integer literal: plain decimal, $hex, or %binary."""
    token = token.strip()
    try:
        if token.startswith('$'):
...
def picbasic_baudmode_to_bps(token):
    """Convert a PicBasic SerOut/SerIn baudmode value to bps, using the
...
def convert_source(text, source_name, fallback_xtal=None):
    ...
    # Rewrite PicBasic-style "Dim NAME [SIZE] As TYPE" arrays and every
    ...
    if reserved_renames:
        header += (
            ...
            "' their PicBasic name collides with a GCBASIC reserved\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert a PicBasic-Pro-style .bas source file to GCBASIC syntax."
    )
